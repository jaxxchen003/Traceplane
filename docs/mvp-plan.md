...
先做一个多 Agent continuity 产品，让多个 Agent 围绕同一条 `episode` 主线共享上下文、过程和产物，并让下一个 Agent 能直接接上工作。
...
3. 每一步关键动作都被记录为 trace 和 artifact
4. 最终 artifact 能回链到来源 memory 和关键过程
5. 下一个 Agent 能通过 brief 和上下文直接继续这条主线
...
`Project -> Episode -> Memory -> Trace -> Artifact -> Brief -> Replay`
